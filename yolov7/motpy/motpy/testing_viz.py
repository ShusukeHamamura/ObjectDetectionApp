import numpy as np
from motpy.utils import track_to_string
...
def draw_rectangle(img, box, color, thickness) -> None:
    ...
    cv2.drawMarker(img, (int(center_x), int(center_y)), color, markerSize=15, thickness=thickness)


def draw_text(img, text, pos, color) -> None:
    tl_pt = (int(pos[0]), int(pos[1]) - 7)
    cv2.putText(img, text, tl_pt, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, thickness=2, color=color)


def draw_track(img, track, ratio, random_color: bool = True, fallback_color=(200, 20, 20), thickness: int = 15, text_at_bottom: bool = False, text_verbose: int = 2): #track: Track
    color = [ord(c) * ord(c) % 256 for c in track.id[:3]] if random_color else fallback_color
    draw_rectangle(img, track.box, color=color, thickness=2)
    ...
    #     draw_text(img, text, pos=pos, color=color)
