...
class KalmanTracker(SingleObjectTracker):
    ...
    def _update_box(self, detection: Detection) -> None: #追跡中のbboxの座標を更新
        z = self.model.box_to_z(detection.box)
        
        self._tracker.update(z) #カルマンフィルタを用いた計算
...
class MultiObjectTracker:
    def __init__(self, dt: float,
        ...
        self.trackers: List[SingleObjectTracker] = [] #現在保存されているbboxを保存している
    ...
    def active_tracks(self,
        ...
        トラッカーのステップ数と古さによるオプションのフィルタリング後に、すべてのアクティブなトラックを返します"""
    ...
    def step(self, detections: Sequence[Detection]) -> List[Track]:
        ...
        アクティブなフィルタリングが適用された後、アクティブなトラックを返します。"""
        t0 = time.time()

        # filter out empty detections(空の検出を除外する)
        detections = [det for det in detections if det.box is not None]

        # predict state in all trackers(全てのトラッカーの状態を予測する) self.tracksは全てのトラッカーを保存している
        for t in self.trackers:
            t.predict()

        # match trackers with detections(トラッカーと検出を一致させる)
        logger.debug('step with %d detections' % len(detections))
        matches = self.matching_fn(self.trackers, detections) #一致したトラッカーと検出のインデックスを配列[トラッカーのインデックス　検出のインデックス]に保存する
        logger.debug('matched %d pairs' % len(matches))

        self.detections_matched_ids = [None] * len(detections)

        # assigned trackers: correct(割り当てられたトラッカーが正しい) トラッカーの座標の更新を行う
        ...
        # not assigned detections: create new trackers POF(割り当てれらていない検出、新しいトラッカーを作成する)
        ...
        for det_idx in set(range(len(detections))).difference(assigned_det_idxs):
            ...
            #print("確認{} , {}".format(det.box, det.class_id))
        ...
        # unassigned trackers(割り当てられていないトラッカー)
        ...
        return_tracks = self.active_tracks(**self.active_tracks_kwargs)
        
        #トラッカーと検出のtracker_idを用いた紐づけ
        for i in range(len(return_tracks)):
            if return_tracks[i].id in self.detections_matched_ids:
                index_num = self.detections_matched_ids.index(return_tracks[i].id)
                return_tracks[i].box[0] = detections[index_num].box[0]
                return_tracks[i].box[1] = detections[index_num].box[1]
                return_tracks[i].box[2] = detections[index_num].box[2]
                return_tracks[i].box[3] = detections[index_num].box[3]
